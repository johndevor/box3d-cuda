"""STRICT humanoid walking-acceptance evaluator. FROZEN.

FROZEN in the same sense as walk/eval/gait.py (the duck judge, whose
structure and criteria this file ports clause-for-clause): thresholds and
semantics may not drift with training; any amendment needs an explicit
authorization note like the contact-debounce AMENDMENT below. The two
judges are comparable artifacts: same criteria set (footfall qualification,
alternation, per-foot balance, support/clearance/placement/slip clauses,
velocity band, stability window, integrity), differing ONLY in
morphology-scaled thresholds (table below) and the up-axis used for tilt.
...
positions, whole-sole heights and contact flags) produced by
FlatFloorHumanoidEnv. The capture's ``tilt_deg`` column is DUCK-frame
(body +Z vs world +Z) and is IGNORED: tilt is recomputed here from
``base_quat_xyzw`` with the humanoid up axis (body +Y vs world +Z,
walk/env/humanoid_native_lane.tilt) — the H0 reset orientation is QX90,
where the duck formula would read 90 degrees.

Acceptance (all three commanded episodes +0.50/+0.75/+1.00 m/s, 8 s each):
  - >= 6 alternating qualified footfalls, >= 3 per foot;
  - a qualified footfall is the touchdown of a swing whose duration is in
    [100 ms, 1.2 s], whose WHOLE sole (min over the 8 box-corner vertices)
    clears >= 30 mm for a contiguous >= 30 ms, with forward (world +x)
    placement >= 150 mm from liftoff to touchdown, continuous support
    >= 80 ms immediately before liftoff and after touchdown, opposite-foot
    support for >= 90% of the swing, and bounded stance slip;
  - stance slip bound: during each stance phase adjacent to the swing, the
    foot COM must stay within STANCE_SLIP_BOUND_M = 100 mm horizontally of
    where that stance began — a planted foot may roll/settle a little but
    must not slide;
  - commanded translation: forward displacement in [60%, 150%] of
    command x 8 s;
  - tilt <= 30 degrees throughout (humanoid up axis);
...
THRESHOLD TABLE (duck value -> humanoid value, one-line justification each;
morphology anchors: hip height ~1.0 m vs ~0.20 m (x5 leg ratio), foot sole
length 0.46 m vs ~0.108 m (x4.3), gait clock 3.33 Hz/mps vs 16.67 (both
encode step length = 1/(2*clock) = their placement floors; the slowest
command gives a 0.6 s cycle for BOTH robots by construction)):
  SWING_MIN_S        0.060 -> 0.100  reward AIR_TIME_MIN; sub-100 ms swings
                                     at 1 m leg scale are contact chatter.
  SWING_MAX_S        1.2   -> 1.2    two slowest-command cycles for both
                                     robots (0.6 s cycle each, see anchors).
  CLEARANCE_M        0.010 -> 0.030  human-scale swing-foot clearance
                                     (3-5 cm); matches reward CLEARANCE_M.
  CLEARANCE_MIN_S    0.020 -> 0.030  same 1/3-of-minimum-swing fraction as
                                     the duck's 20 ms / 60 ms.
  PLACEMENT_MIN_M    0.030 -> 0.150  leg-ratio (x5) scaled minimum step;
                                     equals the clock's encoded step floor
                                     1/(2*3.33) and reward PLACEMENT_MIN_M.
  SUPPORT_S          0.040 -> 0.080  scales with the stance floor (reward
                                     STANCE_MIN_S 0.12 vs duck 0.06); stays
                                     4x the 20 ms debounce (duck was 2x).
  STANCE_SLIP_BOUND_M 0.025 -> 0.100 same ~23% of sole length as the duck's
                                     25 mm / 108 mm (100/460 = 22%).
  CONTACT_DEBOUNCE_S 0.020 -> 0.020  UNCHANGED, amendment carried verbatim
                                     (sensor-physics argument is scale-free
                                     at the shared 2 ms tick).
  MIN_FOOTFALLS 6 / PER_FOOT 3 / OPPOSITE 0.90 / TRANSLATION [0.6, 1.5] /
  TILT 30 deg / FIRST 2.5 s / LAST-WINDOW 1.5 s / 8 s episodes: UNCHANGED —
  dimensionless episode-structure clauses; identical step-rate demand since
  the slowest-command cycle period matches the duck's (anchors above).
"""
...
SCHEMA = "duckgridwalk.episode/1"     # shared capture schema (capture.py)
EPISODE_SECONDS = 8.0
COMMANDS_MPS = (0.50, 0.75, 1.00)     # walk/env/humanoid_flat.COMMANDS_MPS
MIN_FOOTFALLS = 6
MIN_FOOTFALLS_PER_FOOT = 3
SWING_MIN_S = 0.100
SWING_MAX_S = 1.2
CLEARANCE_M = 0.030
CLEARANCE_MIN_S = 0.030
PLACEMENT_MIN_M = 0.150
SUPPORT_S = 0.080
OPPOSITE_SUPPORT_FRACTION = 0.90
STANCE_SLIP_BOUND_M = 0.100   # documented bound, see module docstring
# AMENDMENT (authorized by John, 2026-09-01, carried verbatim from the duck
# evaluator): contact debounce for SUPPORT continuity. The raw flag reports
# single-tick (2 ms) impulse dropouts during soft touchdowns as "lost
# support"; no physical force sensor resolves 2 ms dropouts, and real
# contact estimation is filtered over tens of ms. Gaps of <= 20 ms bracketed
# by contact are treated as continuous contact BEFORE swing and
# support-window analysis. Real swings (>= 100 ms minimum here) are
# unaffected. Every other criterion, threshold, and semantics is unchanged.
...
def humanoid_tilt_deg(quat_xyzw: np.ndarray) -> np.ndarray:
    """Angle [deg] between the humanoid up axis (body +Y) and world +Z.

    up = (R @ [0,1,0])_z = 2*(qy*qz + qx*qw); exactly 0 at the H0 reset
    orientation QX90 (walk/env/humanoid_native_lane.tilt, in degrees)."""
    ...
    for f in range(2):
        for i0, i1 in _runs(~contact[:, f]):
            ...
            if not SWING_MIN_S - 1e-9 <= duration <= SWING_MAX_S + 1e-9:
                why.append("swing duration outside [100 ms, 1.2 s]")
            ...
            if slip > STANCE_SLIP_BOUND_M + 1e-12:
                why.append("stance slip beyond 100 mm bound")
...
def evaluate_episode(trace: dict | str | Path) -> dict:
    ...
    if not ok:
        ...
        return {"schema": "duckgridwalk.humanoid_gait_eval/1", "rejected": True,
    ...
    # HUMANOID up axis: recompute from quats; the capture's duck-frame
    # tilt_deg column is ignored (module docstring).
    tilt = humanoid_tilt_deg(np.asarray(ticks["base_quat_xyzw"], float))
    ...
    return {"schema": "duckgridwalk.humanoid_gait_eval/1", "rejected": False,
            "command_mps": cmd,
...
def evaluate_run(traces: list[dict | str | Path]) -> dict:
    ...
    return {"schema": "duckgridwalk.humanoid_gait_eval_run/1",
