...
def main():
 ...
 suffix='.dylib' if platform.system()=='Darwin' else '.so';lib=out/('libarticulated_v1'+suffix)
 common=['-std=c++17','-Wall','-Wextra','-Werror','-I',str(ROOT/'include'),'-I',str(LANE/'include')]
 sources=[str(LANE/'src/articulated_v1.cpp'),str(ROOT/'csrc/experimental_joint_v1.cpp')]
 try:
  ...
  run('c11-build',[cc,'-std=c11','-Wall','-Wextra','-Werror','-I',str(LANE/'include'),'-I',str(ROOT/'include'),'-I',str(ROOT/'proposals'),str(LANE/'tests/c_header.c'),'-o',str(out/'abi')])
  ...
  env=dict(os.environ,AV1_LIBRARY=str(lib),AV1_METRICS=str(out/'static-reference-metrics.json'),PYTHONDONTWRITEBYTECODE='1')
  run('python-tests',[sys.executable,'-B','-m','unittest','discover','-s',str(LANE/'tests'),'-p','test_articulated_v1.py','-v'],env)
  ...
  result={'schema':'cuda3.articulated-v1.native-validation/v1','all_passed':len(commands)==10 and all(x['exit_code']==0 for x in commands),'commands':commands,'python':sys.version,'platform':platform.platform(),'mujoco_queries':0,'duck_dynamics_steps':0,'provider_calls':0,'cuda_execution_tested':False,'dependencies_installed':0,'files':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in out.iterdir() if p.is_file()}}
