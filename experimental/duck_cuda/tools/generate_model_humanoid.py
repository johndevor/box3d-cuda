...
def load_lowering():
    ...
    import h1_lowering as h0  # noqa: PLC0415  (ACTIVE lowering: H1)
    return h0
...
def emit(h0) -> str:
    ...
    assert np.asarray(reward_mod.REF_GAIT).shape == (
        int(reward_mod.REF_BINS), J), "REF_GAIT shape drift"
    n, q = 6 + J, 7 + J
    ...
    lines = [
        ...
        *[f"#define DW_RW_{name} {float(getattr(reward_mod, name))!r}"
        ...
        + ",".join("{" + ",".join(repr(float(x)) for x in row_vals) + "}"
                   for row_vals in np.asarray(reward_mod.REF_GAIT,
                                              dtype=np.float64)) + "};",
