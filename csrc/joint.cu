...
__device__ inline void basis(const float*a,float*t1,float*t2){float e[3]={0,0,0};e[fabsf(a[0])<.9f?0:1]=1;cross3(e,a,t1);float l=norm3(t1);for(int k=0;k<3;k++)t1[k]/=fmaxf(l,1e-12f);cross3(a,t1,t2);l=norm3(t2);for(int k=0;k<3;k++)t2[k]/=fmaxf(l,1e-12f);}
