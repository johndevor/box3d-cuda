...
constexpr int64_t FACE_TAG=int64_t(1)<<60, EDGE_TAG=int64_t(2)<<60;
...
__device__ inline void solve_normal(float*a,float ima,const float*ia,float*b,float imb,const float*ib,MF*m,MP&q,float e,float eps){float ra[3],rb[3],va[3],vb[3],rel[3];for(int k=0;k<3;k++){ra[k]=q.p[k]-a[k];rb[k]=q.p[k]-b[k];}point_v(a,ra,va);point_v(b,rb,vb);for(int k=0;k<3;k++)rel[k]=vb[k]-va[k];float vn=dot3(rel,m->n),den=emass(a,ima,ia,ra,b,imb,ib,rb,m->n);if(den>eps){float bounce=vn<-.5f?-e*vn:0,delta=(bounce-vn)/den,old=q.jn;q.jn=fmaxf(0,old+delta);float j[3];for(int k=0;k<3;k++)j[k]=m->n[k]*(q.jn-old);pair_impulse(a,ima,ia,b,imb,ib,q,j);}}
      ...
      for(int p=0;p<P;p++)if(active[p]){
        ...
        for(int q=0;q<manif[p].count;q++)
          solve_normal(state+fa*SW,im[fa],ii+fa*3,state+fb*SW,im[fb],ii+fb*3,
              &manif[p],manif[p].points[q],rest,eps);
        for(int q=0;q<manif[p].count;q++)
